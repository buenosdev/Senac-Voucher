...
SELECT COUNT(*) AS "Total de Cidades" FROM CIDADE;
...
SELECT COUNT(*) AS "Total de Setores" FROM SETOR;

-- Quantos pedidos tem no banco?
SELECT COUNT(*) AS "Total de Pedidos" FROM PEDIDO;

-- Qual o maior salário dos vendedores?
SELECT MAX(SALARIO) AS "Maior Salário" FROM VENDEDOR;

-- Quem possui o maior salário?
SELECT NOMEVEND AS "Vendedor com Maior Salário", SALARIO
FROM VENDEDOR
WHERE SALARIO = (SELECT MAX(SALARIO) FROM VENDEDOR);

-- Qual o menor salário dos vendedores?
SELECT MIN(SALARIO) AS "Menor Salário" FROM VENDEDOR;

-- Quem possui o menor salário?
SELECT NOMEVEND AS "Vendedor com Menor Salário", SALARIO
FROM VENDEDOR
WHERE SALARIO = (SELECT MIN(SALARIO) FROM VENDEDOR);

-- Qual pedido teve a maior venda em quantidade?
SELECT NUMPED AS "Pedido com Maior Quantidade", SUM(QTDADE) AS "Quantidade Total"
FROM ITEMPEDIDO
GROUP BY NUMPED
...
SELECT NUMPED AS "Pedido com Menor Quantidade", SUM(QTDADE) AS "Quantidade Total"
FROM ITEMPEDIDO
GROUP BY NUMPED
ORDER BY "Quantidade Total"
LIMIT 1;

-- Qual o produto mais barato?
SELECT DESCRICAO AS "Produto Mais Barato", VALOR_UN
FROM PRODUTO
WHERE VALOR_UN = (SELECT MIN(VALOR_UN) FROM PRODUTO);

-- Qual o produto mais caro?
SELECT DESCRICAO AS "Produto Mais Caro", VALOR_UN
FROM PRODUTO
WHERE VALOR_UN = (SELECT MAX(VALOR_UN) FROM PRODUTO);

-- Quantos vendedores existem no setor de ferramentas?
SELECT COUNT(*) AS "Total de Vendedores no Setor de Ferramentas"
FROM VENDEDOR
...
SELECT COUNT(*) AS "Total de Pedidos do Cliente Estevan Pereira Cardoso"
FROM PEDIDO
